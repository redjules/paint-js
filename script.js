...
window.addEventListener("load", () => {
    ...
    // setting canvas width/height.. offsetwidth/height returns width/height of an element
...
const drawing = (e) => {
    ...
    if(selectedTool === "brush" || selectedTool === "eraser") {
        // if selected tol is eraser then set strokeStyle to white
        // to paint white color on to the existing canvas content else set the stroke color to selected color
        ctx.strokeStyle = selectedTool === "eraser" ? "#fff" : selectedColor;
        ctx.lineTo(e.offsetX, e.offsetY); // creating line according to mouse position to the mouse pointer
...
colorPicker.addEventListener("change", () => {
    // passing picked color value fro color picker to last color btn background
...
saveImg.addEventListener("click", () => {
    ...
    link.download = `${Date.now()}.jpg`; // creating download link
    link.href = canvas.toDataURL(); // creating image data url
    link.click(); // clicking to download link
});
